...
### 3. Configuração das Credenciais de Banco de Dados

Para realizar a conexão com o banco de dados, é necessário criar um arquivo `.env` em formato JSON contendo as configurações de acesso.

O arquivo deve possuir a seguinte estrutura:
...
> **Observação:** mantenha o arquivo `.env` fora do controle de versão (`git`), pois ele contém informações sensíveis de acesso ao banco de dados.

### 4. Execução via Streamlit
...
O projeto utiliza Testes Unitários com a técnica de Mocking para garantir que a lógica de negócio funcione de forma isolada, sem depender de conexões reais com bancos de dados externos durante a fase de testes.
